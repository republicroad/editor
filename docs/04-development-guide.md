...
- **不手工切换行尾**：换行由上述三层配置统一管理；VS Code 新建文件默认 LF
- **提交前自检**：`bun run lint`（含 prettier 规则）应保持 0 errors；若出现行尾报错，先 `npx prettier --write .` 再复查
- **禁止提交 BOM**：所有文本文件一律 UTF-8 无 BOM（严格解析器会因 BOM 报错）
- **json/schema 类数据文件**已列入 `.prettierignore`（运行时数据不做格式化，防 diff 噪音）
