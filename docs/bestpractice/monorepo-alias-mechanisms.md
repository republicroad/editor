...
| 维度 | ①作用域前缀 | ②通用别名+程序隔离 | ③包内全相对 |
|---|---|---|---|
| 碰撞可能性 | 构造性不可能 | 零 | 零 |
| 跨工具一致性 | 四端同前缀各归各解析 | 桥隔离 | 处处成立 |
| 未来 CLI 装机 | 天然兼容 | 随意 | 需再改写（纪律） |
| 子模块一次性成本 | 文本替换 + 配置改名 | **零** | 相对化改写 |
| 维护心智 | "内核的东西都带 `@kernel`" | "内核是黑盒" | "内核无特权" |
| 业界对应 | Nx | Composite/声明消费 | Turbo |
...
内核包内          @kernel/icons、@kernel/lib/utils ← Nx 式作用域别名
appshell 包内     相对导入（../../lib/...）         ← Turbo 式无别名
宿主 app 内       @/components/ui/...              ← shadcn 单 app 惯例（reui 装机自由）
```

- 解析矩阵：`@kernel/*` 在根 tsconfig 与内核 tsconfig 双侧同名映射（都指向内核
  src），tsc/vite/bun/storybook 四端一致；bun 测试 mock 绑定要求**同一 specifier
  在所有 tsconfig 域解析到同一路径**（见多包构建篇 §6.2）。
...
| 第四十六批 | 内核 0.3.0 发布 → **执行 A′（模型①）** | 发布边界 = 跨仓重构的协调窗口；0.3.0 别名面零变动，窗口最干净 |
| 回滚预案 | 模型②保留为备案 | 若前缀化中途发现不可调和项，桥可即时恢复（工件与脚本在 git 历史） |
